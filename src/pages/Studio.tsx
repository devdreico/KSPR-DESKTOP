...
import { CliParityPanel } from "../components/CliParityPanel";
import {
  checkProviderStatus,
  ingestArchive,
  ingestPath,
...
const COMMANDS: CommandItem[] = [
  ...
  { id: "api", title: "API y proveedores", description: "Configura APIs y lista modelos disponibles", slash: "/api" },
  { id: "login", title: "Autenticación CLI", description: "Estado de la cuenta y acceso del agente", slash: "/login" },
  { id: "project", title: "Gestor de proyectos", description: "Crea o abre un workspace local", slash: "/project" },
  { id: "cli", title: "Comandos de KSPR CLI", description: "MCP, plugins, skills, capacidades y árboles", slash: "/cli" },
  ...
  { id: "plugins", title: "Gestionar plugins", description: "Lista y carga plugins de ~/.kspr/plugins", slash: "/plugins" },
  { id: "capabilities", title: "Descubrir capacidades", description: "Busca harnesses CLI-Anything instalados", slash: "/capabilities" },
  { id: "skills", title: "Consultar skills", description: "Inspecciona bundles de instrucciones del agente", slash: "/skills" },
  { id: "prompts", title: "Prompts guardados", description: "Gestiona prompts persistentes de KSPR", slash: "/prompts" },
  { id: "decompilate", title: "Generar Context Tree", description: "Indexa archivos, PDFs, imágenes o enlaces", slash: "/decompilate" },
  { id: "trees", title: "Listar Context Trees", description: "Abre los árboles de contexto generados", slash: "/trees" },
  { id: "update", title: "Actualizar KSPR", description: "Muestra el instalador oficial de la CLI", slash: "/update" },
...
export function Studio() {
  ...
  const [cliParityOpen, setCliParityOpen] = useState(false);
  const [cliParityCommand, setCliParityCommand] = useState("plugins");
  ...
  useEffect(() => {
    let cancelled = false;
    void listSessions().then(({ sessions: remote }) => {
      if (cancelled || !remote?.length) return;
      const normalized = remote.map((item) => item as SessionRecord);
      setSessions((current) => {
        const local = new Map(current.map((session) => [session.id, session]));
        normalized.forEach((session) => local.set(session.id, { ...local.get(session.id), ...session }));
        return [...local.values()].sort((a, b) => b.updatedAt - a.updatedAt).slice(0, 50);
      });
    }).catch(() => {
      // El runtime puede seguir bloqueado hasta introducir la licencia.
    });
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void saveSession(active).catch(() => {
        // La copia local sigue disponible si el servidor está bloqueado o desconectado.
      });
    }, 350);
  ...
  useEffect(() => {
    ...
    const defaultBaseUrl = providerKey === "groq" ? "https://api.groq.com/openai/v1" : providerKey === "deepseek" ? "https://api.deepseek.com/v1" : providerKey === "anthropic" ? "https://api.anthropic.com/v1" : providerKey === "openrouter" ? "https://openrouter.ai/api/v1" : providerKey === "opencode-zen" ? "https://zen.opencode.ai/api/v1" : compatibleBaseUrl;
    setConnectBaseUrl(providerConfigs[providerKey]?.baseUrl || defaultBaseUrl);
  ...
  function importFilesIntoSession(loaded: SourceFile[]) {
    setFiles((current) => [...current.filter((item) => !loaded.some((incoming) => incoming.path === item.path)), ...loaded]);
    setContextFiles((current) => [...current.filter((item) => !loaded.some((incoming) => incoming.path === item.path)), ...loaded]);
  }
  ...
  function executeCommand(commandId: string) {
    ...
    switch (commandId) {
      ...
      case "api": setApiHubOpen(true); break;
      case "provider": setApiHubOpen(true); break;
      case "login": setCliParityCommand("login"); setCliParityOpen(true); break;
      case "project": void selectLocalFolder(); break;
      case "cli": setCliParityCommand("plugins"); setCliParityOpen(true); break;
      case "config": setConfigModalOpen(true); break;
      case "agents": setAgentModalOpen(true); break;
      case "mcp": setMcpModalOpen(true); break;
      case "plugins": setCliParityCommand("plugins"); setCliParityOpen(true); break;
      case "capabilities": setCliParityCommand("capabilities"); setCliParityOpen(true); break;
      case "skills": setCliParityCommand("skills"); setCliParityOpen(true); break;
      case "prompts": setCliParityCommand("prompts"); setCliParityOpen(true); break;
      case "decompilate": setCliParityCommand("decompilate"); setCliParityOpen(true); break;
      case "trees": setCliParityCommand("trees"); setCliParityOpen(true); break;
      case "update": setCliParityCommand("update"); setCliParityOpen(true); break;
  ...
  async function connectSelectedProvider() {
    ...
    try {
      ...
      const providerName = connectProvider === "gemini" ? "Google Gemini" : connectProvider === "openai-compatible" ? "OpenAI-compatible" : connectProvider === "custom" ? customProviderName.trim() || providerKey : providerKey;
  ...
  return (
    <div className={`app-shell app-shell-v2 ${sidebarOpen ? "sidebar-is-open" : "sidebar-is-closed"}`}>
            ...
            <div className="api-hub-section" style={{ marginBottom: "16px" }}>
              ...
              <div className="provider-cards">
                ...
                {[
                  ...
                  ["opencode-zen", "OpenCode Zen", "OpenAI-compatible"],
                ].map(([id, name, description]) => <button key={id} className={connectProvider === id ? "provider-card active" : "provider-card"} onClick={() => { setConnectProvider(id); setConnectBaseUrl(id === "groq" ? "https://api.groq.com/openai/v1" : id === "deepseek" ? "https://api.deepseek.com/v1" : id === "anthropic" ? "https://api.anthropic.com/v1" : id === "openrouter" ? "https://openrouter.ai/api/v1" : id === "opencode-zen" ? "https://zen.opencode.ai/api/v1" : "https://api.openai.com/v1"); }}>
                  <Terminal size={17} />
                  <span><strong>{name}</strong><small>{description}</small></span>
                  {connectProvider === id && <Check size={14} />}
                </button>)}
            ...
            <div className="api-hub-section" style={{ borderTop: "1px solid var(--line)", paddingTop: "14px" }}>
              ...
              {!customModelOpen ? (
                ...
                <div className="custom-model-form">
                  <div className="config-grid">
                    <div className="modal-field"><label>Proveedor</label><select value={customModelProvider} onChange={(event) => setCustomModelProvider(event.target.value)}>{[["openai-compatible", "OpenAI-compatible"], ["openai", "OpenAI"], ["groq", "Groq"], ["deepseek", "DeepSeek"], ["anthropic", "Anthropic"], ["openrouter", "OpenRouter"], ["opencode-zen", "OpenCode Zen"], ["gemini", "Google Gemini"], ["local", "KSPR Local"]].map(([id, name]) => <option key={id} value={id}>{name}</option>)}</select></div>
      ...
      {cliParityOpen && <CliParityPanel initialCommand={cliParityCommand} mcpServers={mcpServers} onImportFiles={importFilesIntoSession} onClose={() => setCliParityOpen(false)} />}
    </div>
  );
}
